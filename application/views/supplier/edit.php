<main role="main" class="container">
	<form method="POST" action="<?php echo base_url();?>index.php/supplier/aksiedit"> 
		<table class="table">
			<tr>
				<td>
				Kode Supplier</td>
				<td>:</td>
				<td><input type="text" size="30" name="kodesupplier" value="<?php echo $dtl->kodesupplier;?>" readOnly=true/></td>
			</tr>
			<tr>
				<td>
				Nama Supplier</td>
				<td>:</td>
				<td><input type="text" class="form-control" size="30" name="namasupplier" value="<?php echo $dtl->namasupplier;?>"/></td>
			</tr>
			<tr>
				<td>
				Alamat Supplier</td>
				<td>:</td>
				<td><input type="text" class="form-control" size="30" name="alamat" value="<?php echo $dtl->alamat;?>"/></td>
			</tr>
			<tr>
				<td>
				No Telp</td>
				<td>:</td>
				<td><input type="text" class="form-control" size="30" name="notelp" value="<?php echo $dtl->notelp;?>"/></td>
			</tr>
			<tr>
				<td>
				Email</td>
				<td>:</td>
				<td><input type="text" class="form-control" size="30" name="email" value="<?php echo $dtl->email;?>" /></td>
			</tr>
		</table>
		<input type="submit" class="btn btn-primary" value="Simpan" />
		</form>
		</p>
</main>